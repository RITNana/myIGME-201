...
namespace Mandelbrot
{
    ...
    class Class1
    {
        ...
        static void Main(string[] args)
        {
            ...
            // Solution for Mandlebrot Set by Professor:

            double icStart, icEnd;
            double rcStart, rcEnd;
            double icIncrement, rcIncrement;
            bool bValid = false;
            do
            {
                do
                {
                    Console.Write("Please enter a starting value for imagCoord (default: 1.2):");
                } while (!double.TryParse(Console.ReadLine(), out icStart));
                do
                {
                    Console.Write("Please enter an ending value for imagCoord (default: -1.2):");


                } while (!double.TryParse(Console.ReadLine(), out icEnd));
                if (icStart <= icEnd)
                {
                    Console.WriteLine("The first number should be GREATER than the second number, please try again.");
                    Console.WriteLine();
                    bValid = false;
                    }
                    else
                ...
                // calculate the increment so that there will be 48 lines in the image
                icIncrement = (icStart - icEnd) / 48;
                Console.WriteLine();
                do
                {
                    do
                    {
                        Console.Write("Please enter a starting value for realCoord (default: - 0.6): ");
                    } while (!double.TryParse(Console.ReadLine(), out rcStart));
                    do
                    {
                        Console.Write("Please enter an ending value for realCoord (default: 1.77): ");
                    } while (!double.TryParse(Console.ReadLine(), out rcEnd));
                    if (rcStart >= rcEnd)
                    {
                        Console.WriteLine("The first number should be LESS than the second number, please try again.");
                        Console.WriteLine();
                        bValid = false;
                        }
else
                    ...
                    // calculate the increment so that there will be 80 characters per line of the image
                    rcIncrement = (rcEnd - rcStart) / 80;
                    for (imagCoord = icStart; imagCoord >= icEnd; imagCoord -= icIncrement)
                    {
                        for (realCoord = rcStart; realCoord <= rcEnd; realCoord += rcIncrement)
                        {
                    iterations = 0;
                    realTemp = realCoord;
                    imagTemp = imagCoord;
